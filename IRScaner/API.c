...
// ƒлина буфера стандартных частот
...
void SetCarrierFrequency(const uint32_t value)
{
	...
	period = (uint16_t) (SystemCoreClock / value) - 1;  // ѕериод
...
static void InitCarrierTimer(void)
{
	...
	uint16_t period = (uint16_t) (SystemCoreClock / 30000) - 1;  // ѕериод
